<section class="model container indent">
    ...
    <div class="model__wrapper">
        @foreach(($data['images_1'] ?? []) as $img)
            ...
            <img src="{{ str_starts_with($file, 'blocks/') ? asset('storage/' . $file) : asset('img/' . $file) }}" alt="{{ $data['title_1'] ?? '' }}">
        @endforeach
    </div>
    ...
    <div class="model__wrapper">
        @foreach(($data['images_2'] ?? []) as $img)
            @php($file = is_array($img) ? ($img['value'] ?? $img['img'] ?? '') : $img)
            <img src="{{ str_starts_with($file, 'blocks/') ? asset('storage/' . $file) : asset('img/' . $file) }}" alt="{{ $data['title_2'] ?? '' }}">
        @endforeach
    </div>
</section>
